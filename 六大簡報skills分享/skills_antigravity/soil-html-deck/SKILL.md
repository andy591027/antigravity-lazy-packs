...
name: soil-html-deck
description: >
  HTML 簡報技能（自由度天花板，又名 HTML簡報）。用單一獨立的 .html 檔案模擬簡報的呈現邏輯,
  圖像由 Antigravity 全域 draw 技能生成並 base64 內嵌,排版/文字/互動全部由 HTML + CSS + JS 處理,
  支援互動圖表(Chart.js)、可點擊表格、影片嵌入、RWD 跨裝置、一鍵分享 URL。
  當使用者說「做 HTML 簡報」、「HTML簡報」、「網頁版簡報」、「單一獨立 .html 檔案」、
  ...
  本技能輸出是單一獨立 .html 檔案,自由度最高、可嵌互動、可立即線上分享,
  但需要瀏覽器環境播放,不適合給只用 PowerPoint 的對象。
---

# SOIL HTML 簡報工作流(soil-html-deck)

以 SOIL 教學設計邏輯為骨架,用 **HTML + Antigravity draw 技能圖像 + JS 互動** 產出單一 `.html` 簡報檔。
...
**所有圖像呼叫一次性平行發出**,不要序列等待。呼叫 `draw.py` 預設參數:
- `--quality low`(由 Google AI Studio 提供高效、經濟且支援免費額度的 `gemini-2.5-flash-image` 驅動，99% 場景夠用)
- `--size 1536x1024`(滿版/對比圖，自動映射至 16:9 比例)或 `1024x1024`(類型卡，自動映射至 1:1)
- 存到 `generated/` 
...
理由:在小型預覽面板(如 Antigravity / vscode 預覽)會被縮成極小,字會擠成一團,觀感極差。
...
## 給 Antigravity Agent 的呼叫提示

如本技能被 Antigravity Agent 讀取使用,請遵守:
...
| 对比頁只有表格太枯燥 | 左側放「左舊右新」對比視覺圖,右側放可排序表 |
| 標題太長破版 | 嚴守 ≤10 字原則 |
