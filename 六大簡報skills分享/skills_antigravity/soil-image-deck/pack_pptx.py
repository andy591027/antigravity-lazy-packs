"""
soil-image-deck 打包腳本 (Antigravity 最佳化版)
...
def pack_baked(images_dir: Path, output: Path):
    ...
    if not pngs:
        # 如果是 generated/ 目錄，也找看看
        pngs = sorted(glob.glob(str(images_dir / "*.png")))
        if not pngs:
            raise SystemExit(f"錯誤：{images_dir} 找不到任何 PNG 圖片")

    by_page = {}
    for p in pngs:
        # 依照前綴排序，支援 page_01 或類似檔名格式
        name = Path(p).name
...
def main():
    ...
    p.add_argument("--images-dir", default="generated", help="圖片目錄")
    p.add_argument("--output", default="output.pptx", help="輸出 pptx")
